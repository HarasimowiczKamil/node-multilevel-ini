...
PATT_COMMENT_LINE = /^;.*$/gi,
PATT_TITLE_LINE = /^\[([a-zA-Z0-9_\-]+)\s*(:\s*([a-zA-Z0-9_\-]+))?\s*\]$/,
PATT_DATA_LINE = /^([a-zA-Z0-9\._\-]+)(\[\])?\s*=\s*(.*)$/,
PATT_SLASH = /(\\+)$/;
 ...
 * @param {Object} chunk
 * @param {Boolen} firstElementIsTitle
 * @param {String} prefix
 *
 * @return {String}
 */
...
function iniToObj(str) {
    var endIndex, line, localResultPoint, matches, levels,
    result = {},
    resultPoint = result,
    lines = str.split('\n');
    for (var i=0; i < lines.length; i++) {
        line = lines[i].replace(/(\r$|^\s*|\s*$)/gi, '');
        localResultPoint = resultPoint;
 ...
 * @return {object}
 */
function getSync(filePath) {
    var data = fs.readFileSync(filePath, 'utf-8');
    if (data) {
        return iniToObj(data);
    }
    return false;
};
